...
class Dropout(object):
    def __init__(self, input, label,\
                 inputNum, hidden_lyrsNeuros, ouputNum,\
        ...
        self.hidden_layers = []
        self.n_layers = len(hidden_lyrsNeuros) # [10, 10]
        ...
        for i in range(self.n_layers):
            ...
            if i == 0:
                ...
                input_size = hidden_lyrsNeuros[i-1]
            ...
            self.log_layer = LogisticRegression(input=self.hidden_layers[-1].output(),
                                                label=self.y,
                                                inputNum=hidden_lyrsNeuros[-1],
                                                ouputNum=ouputNum)


    def train(self, epochs=5000, dropout=True, ratioDrop=0.5, rng=None):
        ...
        for epoch in range(epochs):
            ...
            for i in range(self.n_layers):
                ...
                if dropout == True:
                    mask = self.hidden_layers[i].dropout(input=layer_input, p=ratioDrop, rng=rng)
                    layer_input *= mask
    ...
    def predict(self, x, dropout=True, ratioDrop=0.5):
        layer_input = x

        for i in range(self.n_layers):
            if dropout == True:
                self.hidden_layers[i].W = ratioDrop * self.hidden_layers[i].W
                self.hidden_layers[i].b = ratioDrop * self.hidden_layers[i].b
...
class HiddenLayer(object):
    def __init__(self, input, inputNum, ouputNum,\
        ...
        if W is None:
            ...
            W = numpy.array(rng.uniform(  # initialize W uniformly
                low=-a,
                high=a,
                size=(inputNum, ouputNum)))

        if b is None:
            b = numpy.zeros(ouputNum)  # initialize bias 0
...
class LogisticRegression(object):
    def __init__(self, input, label, inputNum, ouputNum):
        self.x = input
        self.y = label
        self.W = numpy.zeros((inputNum, ouputNum))  # initialize W 0
        self.b = numpy.zeros(ouputNum)          # initialize bias 0
...
def test_dropout(n_epochs=5000, dropout=True, ratioDrop=0.5):
    ...
    classifier = Dropout(input=x, label=y, \
                         inputNum=2, hidden_lyrsNeuros=[10, 10], ouputNum=2, \
                         rng=rng, activation=ReLU)


    # train
    classifier.train(epochs=n_epochs, dropout=dropout, \
                     ratioDrop=ratioDrop, rng=rng)
